...
async def coinflip(bot, ctx):
    msg = await ctx.send(f"{ctx.author} | Guess whether the coin will land on heads or tails.")
    use_msg = False
    winnings = None
    if ctx.channel.permissions_for(ctx.author).add_reactions:
        try:
            ...
            await msg.edit(content=f"{ctx.author} | Guess whether the coin will land on heads or tails.\n\n**I don't have add_reactions permissions, say h/heads or t/tails.**")
            use_msg = True
    else:
        use_msg = True
    ...
    if use_msg:
        ...
        try:
            ...
            await ctx.send(f"{ctx.author} | You took too long to respond, you lose.")
            return

        answer = random.choice((("h", "heads"), ("t", "tails")))
        if guess.content.lower() in answer:
            winnings = random.randint(10, 25)
            await ctx.send(f"{ctx.author} | You won {winnings} pet coins! The coin landed on {answer[1]}. \U0001f389")
        else:
            await ctx.send(f"{ctx.author} | You didn't guess correctly! The coin landed on {answer[1]}. \U0001f626")
    else:
